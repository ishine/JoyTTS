...
class CosyVoiceModel:
    ...
    def load(self, llm_model, flow_model, hift_model):
        ...
        if "epoch" in llm_state_dict:
            epoch = llm_state_dict.pop("epoch")
            step = llm_state_dict.pop("step")
    ...
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid, prompt_text_token_emb=None, stream=False):
        with self.llm_context, torch.cuda.amp.autocast(self.fp16):
            if isinstance(text, Generator):
                assert isinstance(self, CosyVoice2Model), 'streaming input text is only implemented for CosyVoice2!'
                for i in self.llm.inference_bistream2(text=text,
                ...
                if stream:
                    if hasattr(self.llm, 'vllm'):
                        for i, text_answer in self.llm.inference_end2end_vllm(text=text,
                                                    prompt_text=prompt_text,
                                                    prompt_text_token_emb=prompt_text_token_emb,
                                                    prompt_speech_token=llm_prompt_speech_token.to(self.device),
                                                    prompt_speech_token_len=torch.tensor([llm_prompt_speech_token.shape[1]], dtype=torch.int32).to(self.device),
                                                    embedding=llm_embedding.to(self.device)):
                            self.tts_speech_token_dict[uuid].append(i)
                        ...
                        for i, text_answer in self.llm.inference_end2end_bistream(text=text,
                                                    prompt_text=prompt_text,
                                                    prompt_text_token_emb=prompt_text_token_emb,
                                                    prompt_speech_token=llm_prompt_speech_token.to(self.device),
                                                    prompt_speech_token_len=torch.tensor([llm_prompt_speech_token.shape[1]], dtype=torch.int32).to(self.device),
                    ...
                    for i, text_answer in self.llm.inference_end2end(text=text,
                                                prompt_text=prompt_text,
                                                prompt_text_token_emb=prompt_text_token_emb,
                                                prompt_speech_token=llm_prompt_speech_token.to(self.device),
                                                prompt_speech_token_len=torch.tensor([llm_prompt_speech_token.shape[1]], dtype=torch.int32).to(self.device),
                ...
                for i in self.llm.inference(text=text,
                                            prompt_text=prompt_text,
                                            prompt_speech_token=llm_prompt_speech_token.to(self.device),
                                            prompt_speech_token_len=torch.tensor([llm_prompt_speech_token.shape[1]], dtype=torch.int32).to(self.device),
                                            embedding=llm_embedding.to(self.device)):
    ...
    def tts(self, text='', flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
            prompt_text='',
...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def __init__(self,
        ...
        # self.token_hop_len = 15 #调小以后，首音频可能是空，不要再调小了，保持25
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
            ...
            prompt_text_token_emb=torch.zeros(1, 0, dtype=torch.float32),
            llm_prompt_speech_token=torch.zeros(1, 0, dtype=torch.int32),
            flow_prompt_speech_token=torch.zeros(1, 0, dtype=torch.int32),
            prompt_speech_feat=torch.zeros(1, 0, 80), source_speech_token=torch.zeros(1, 0, dtype=torch.int32), stream=False, speed=1.0, this_uuid=None, **kwargs):
        # this_uuid is used to track variables related to this inference thread
        this_uuid = str(uuid.uuid1()) if this_uuid is None else this_uuid
        with self.lock:
            self.tts_speech_token_dict[this_uuid], self.llm_end_dict[this_uuid] = [], False
            self.hift_cache_dict[this_uuid] = None
            self.flow_cache_dict[this_uuid] = self.init_flow_cache()
        if source_speech_token.shape[1] == 0:
            p = threading.Thread(target=self.llm_job, args=(text, prompt_text, llm_prompt_speech_token, llm_embedding, this_uuid, prompt_text_token_emb, stream))
        else:
            p = threading.Thread(target=self.vc_job, args=(source_speech_token, this_uuid))
        p.start()
        if stream is True:
            ...
            while True:
                ...
                if len(self.tts_speech_token_dict[this_uuid]) >= self.token_hop_len + self.flow.pre_lookahead_len:
                    ...
                    yield {'tts_speech': this_tts_speech.cpu(), 'text_answer':self.text_answer}
                    with self.lock:
    ...
    def extract_speech_token(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
            prompt_text=torch.zeros(1, 0, dtype=torch.int32),
            llm_prompt_speech_token=torch.zeros(1, 0, dtype=torch.int32),
            flow_prompt_speech_token=torch.zeros(1, 0, dtype=torch.int32),
            prompt_speech_feat=torch.zeros(1, 0, 80), source_speech_token=torch.zeros(1, 0, dtype=torch.int32), stream=False, speed=1.0, save_wav=False, **kwargs):
        # this_uuid is used to track variables related to this inference thread
        this_uuid = str(uuid.uuid1())
        with self.lock:
            self.tts_speech_token_dict[this_uuid], self.llm_end_dict[this_uuid] = [], False
        ...
        if save_wav:
            this_tts_speech = self.token2wav(token=this_tts_speech_token,
                                                ...
                                                finalize=True,
                                                speed=speed)
            yield {'speech_token': this_tts_speech_token.cpu(), 'tts_speech': this_tts_speech.cpu()}
        else:
            yield {'speech_token': this_tts_speech_token.cpu()}
        with self.lock:
            self.tts_speech_token_dict.pop(this_uuid)
            self.llm_end_dict.pop(this_uuid)
            self.hift_cache_dict.pop(this_uuid)

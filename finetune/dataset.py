...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            if "image" not in self.raw_data[i]:
                images_dict = {}
            elif isinstance(self.raw_data[i]["image"], str):
            ...
            conversations = [{"role": "user","content": self.raw_data[i]["system"]}] + self.raw_data[i]["conversations"] if "system" in self.raw_data[i] \
                            else self.raw_data[i]["conversations"]
            conversations = [{"role": "user","content": self.raw_data[i]["reference"]}] + self.raw_data[i]["conversations"] if "reference" in self.raw_data[i] \
                            else self.raw_data[i]["conversations"]
            ret = preprocess(
                images_dict,
                conversations,
